...
# Public source files already used by the project.
DRIVE={
...
def clean_props(a):
 keep={'objectid','OBJECTID','we_nr','lgbez','typ','plz','ort','strasse','nr','bl','flaeche_qm','anzahl_gebaeude','sto','stober','GEN','gen','NAME','name','BEZ','bez','AGS','ags','ARS','ars','RS','rs','SN_L','SN_R','EWZ','ewz'}
...
def arc_geometry(rec,name):
 ...
 for ring in rings:
  try:
   ...
   p=transform(merc,p).simplify(SIMPLIFY[name],preserve_topology=True)
...
def geojson_geometry(rec,name):
 try:
  ...
  # Administrative source files are expected in WGS84.
...
def build(name,fid,mode):
 ...
 path=f'data/{name}.geojson'
 with open(path,'w',encoding='utf-8') as f:
  json.dump({'type':'FeatureCollection','features':out},f,ensure_ascii=False,separators=(',',':'))
 print(name,len(out),os.path.getsize(path))

for name,(fid,mode) in DRIVE.items():
 build(name,fid,mode)
